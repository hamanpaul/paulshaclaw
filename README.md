...
- Stage 2 — Memory Atomizer/Linker（Topic 3，已落地）+ T3.2 LLM 語意晉升（**Phase 2a forward-only canary 已啟用**）
  ...
  - T3.2 / Phase 2a：把 `Promoter` 換成 LLM 語意實作（per-session 語意拆分/合併 + tag + session 內/已知實體 relations；跨 session 演進脈絡與全域實體圖留 Topic 5）。dream loop 已翻 `--promoter llm`（forward-only canary），atomize 輸出窗由 `agent_exec.max_output_tokens`（預設 8192）控制、MOC 升級為 session 標題主脊 + 巢狀原子雙層；全量回填重蒸餾留 Phase 2b。設計：[`docs/superpowers/specs/2026-06-17-stage2-phase2-llm-promoter-design.md`](./docs/superpowers/specs/2026-06-17-stage2-phase2-llm-promoter-design.md)。
