...
- `psc deck compile <combo> --task "..." --change <name> [--emit]`：一句 task + 一張 combo（卡片 glob 用到 `<change>` 佔位符時必須帶 `--change`） → 展開成 manager 可代管的 slice specs（interactive 卡編為前置 checklist、headless 卡依 `slice_group` 合併為 slices + `depends_on` DAG）。
- 安全預設：預設 dry-run；`--emit` 一律 `dispatch: hold`（翻 `auto` 是人的動作）；同名拒絕、`--force` 才原子覆蓋。
- `psc deck verify <card-id> --task-slug <slug> [--change <name>]`：卡片 produces glob 存在性驗收（翻 auto 前的人工 checklist；exit 0=pass、1=fail）。
- 使用者指定卡以 `--with card[:after=<id>|:before=<id>]` **加入手牌不取代骨幹**；`--only` 才排他。
