...
if [[ -z "${PY:-}" ]]; then
  # Prefer PSC_PYTHON / system python3 over the repo .venv.
  PY="$(command -v "${PSC_PYTHON:-python3}" 2>/dev/null || true)"
  if [[ -z "$PY" ]]; then
    PY="$REPO/.venv/bin/python"
    [[ -x "$PY" ]] || { echo "找不到可用的 python（設 PSC_PYTHON 或於 repo 執行 'python3 -m venv .venv && .venv/bin/python -m pip install --upgrade --force-reinstall -e .'）" >&2; exit 1; }
  fi
fi
