...
- 新 repo 首發為 **private**，但 day-1 即達 **public-ready**：宣告 `tier: shareable` + 全檔去識別化；實際轉 public 為獨立 track（呼應 repo-public-migration 謹慎）。
- **去識別化（R-21，公開就緒裁決 2026-07-07）**：所有檔案（平移的 code、tests、scripts、docs）MUST NOT 含個人絕對路徑（`/home/paul_chen`、`paul_chen`）、雇主／廠商標記；一律改用 `$HOME`／env／相對路徑或佔位符。R-21 由 conventions 引擎承接（不自製 scanner）；tier: shareable 宣告後引擎自動掃描。
- conventions 引擎 pin：template 現為 **v1.0.7**（`e24fbd679d35d04a79ea21aff7733fadebd5e77e`），MUST bump 至 **v1.0.12**（`58290153a400926851afa0f1794236e7669847c6`，與主 repo `policy-check.yml` 相同）；`.paul-project.yml` 與 `policy-check.yml` 的 `policy_version` 兩處同步為 `1.0.12`（R-20，無豁免）。
- **升版流程（硬前置）**：bump 引擎後 MUST 先在本機以 1.0.12 引擎實跑 policy-check 確認**零 fail** 才 push（本地舊版引擎測不到新規則）。
- agent 檔採 symlink 模式（R-14）：AGENTS.md／GEMINI.md／.github/copilot-instructions.md → CLAUDE.md（symlink 已在 template 就位，勿改為 copy）。
...
- [ ] **Step 4: 宣告 tier + bump 引擎 1.0.7→1.0.12（R-20 兩處同步）**

`.paul-project.yml`：補 `tier: shareable`；`policy_version: 1.0.7` → `1.0.12`；`code_paths` 增補 `paulsha_cortex/**`、`tests/**`、`pyproject.toml`。
`.github/workflows/policy-check.yml`：pin `@e24fbd679d35d04a79ea21aff7733fadebd5e77e  # v1.0.7` → `@58290153a400926851afa0f1794236e7669847c6  # v1.0.12`；`policy_version: "1.0.7"` → `"1.0.12"`。

```bash
grep -rn 'policy_version\|reusable-policy-check' .paul-project.yml .github/workflows/policy-check.yml
```

Expected: 兩檔 `policy_version` 皆 `1.0.12`、pin 為 `@58290153…`，無殘留 `e24fbd67`/`1.0.7`（R-20 一致）。

- [ ] **Step 5: Commit**

```bash
git add -A && git commit -m "chore: paulsha-cortex 骨架（pyproject/tier:shareable/引擎 bump 1.0.12）"
```
...
- Create: `paulsha_cortex/scripts/{psc-relay-hook.sh,coordinator_telegram_notifier.py}`、`paulsha_cortex/scripts/hooks/{claude,codex,copilot}.json`
...
- [ ] **Step 1: 平移 coordinator runtime scripts（僅乾淨的三類；排除項見下）**

只搬 coordinator runtime 需要的檔；**不搬** `dispatch-stage-wave-a.sh`／`copilot-stage-worker.sh`（paulshaclaw 自身派工工具、含個人路徑，非 cortex runtime，留主 repo）、`install-manager-units.sh`（已由 Task 8 installer 取代）。

```bash
cd ~/prj_pri/paulsha-cortex && mkdir -p paulsha_cortex/scripts/hooks
cp /tmp/claw-src/scripts/coordinator/psc-relay-hook.sh \
   /tmp/claw-src/scripts/coordinator/coordinator_telegram_notifier.py paulsha_cortex/scripts/
cp /tmp/claw-src/scripts/coordinator/hooks/*.json paulsha_cortex/scripts/hooks/
```

驗證搬入檔無個人路徑（tier: shareable 前置）：

```bash
grep -rnE '/home/paul_chen|paul_chen' paulsha_cortex/scripts/ && echo 'FOUND — 需去識別化' || echo CLEAN
```

Expected: `CLEAN`（psc-relay-hook.sh 用 `$HOME`／`PSC_*`，notifier 與 hooks json 無個人路徑——已 2026-07-07 核對）。
...
- [ ] **Step 3: pre-push 去識別化掃描（tier: shareable 硬前置）**

全 repo 掃個人絕對路徑／使用者名（R-21 範圍）；命中即就地改 `$HOME`／env／佔位符後重掃至 CLEAN。

```bash
grep -rnE '/home/paul_chen|paul_chen' --include='*.py' --include='*.sh' --include='*.md' \
  --include='*.json' --include='*.yml' --include='*.yaml' \
  ~/prj_pri/paulsha-cortex/paulsha_cortex ~/prj_pri/paulsha-cortex/tests \
  ~/prj_pri/paulsha-cortex/docs 2>/dev/null && echo 'FOUND — 去識別化後重掃' || echo CLEAN
```

Expected: 最終 `CLEAN`。（測試 fixture 常寫死家目錄——改用 `tmp_path`／`Path.home()`／monkeypatch，勿留絕對路徑。）

- [ ] **Step 4: 本機以 1.0.12 引擎實跑 policy-check（升版流程硬前置）**

依 conventions 引擎 README 於本機以 v1.0.12 實跑（reusable workflow 的本地等價命令）；確認 **零 fail** 才可 push。R-21（tier: shareable 掃描）、R-20（policy_version 一致）、R-14（symlink 模式）皆須綠。

Expected: policy-check 本機零 fail；若 R-21 命中殘留個人路徑，回 Step 3。

- [ ] **Step 5: push、開 PR、merge**
...
PR body 引用主 repo `hamanpaul/paulshaclaw#232`（跨 repo 引用，掛 `policy-exempt:issue-link`；R-09 code PR 掛 `skip-changelog`）。**label 若未進 opened event payload 被 policy 看到，用 REST API `issues/<n>/labels` 補、必要時 close/reopen 刷新 payload**（deck 管線教訓）。policy 綠後 merge。
