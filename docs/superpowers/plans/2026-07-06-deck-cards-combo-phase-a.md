...
轉錄真相源：`~/.agents/skills/mcu-coding-skill/SKILL.md`（TDD/subagent/requesting-code-review/**receiving-code-review**/verification 五個 REQUIRED SUB-SKILL 全數對應；硬體證據規則 → 獨立 interactive 卡）。若轉錄中發現 schema 表達不了的結構，回饋 Task 1 修 schema（勿硬塞）。
