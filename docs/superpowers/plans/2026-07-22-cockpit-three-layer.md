...
**Tech Stack:** Python 3.12、textual 0.61.1（**不升版**）、unittest（`python -m unittest`）、tmux 3.4。

## Global Constraints

- textual 釘在 0.61.1；只用 0.61 已支援的 tcss 屬性（`max-height`/`min-height`/`overflow-y` 皆可）
- repo 未 pip install：測試從 repo root 跑 `python -m unittest ...`（依賴 `paulsha_cortex` 已裝於環境）
...
- 每個 Task 結束時全測試套件必須綠燈：`python -m unittest discover -s tests -v 2>&1 | tail -5`
...
Run: `python -m unittest tests.test_stage11_operator_cockpit -v 2>&1 | tail -8`
...
class FakeActions:
    """錄呼叫序的 layout actions；fail_first_swap=True 時第一次 swap 拋錯。"""
    ...
    def swap_selected_with_active(self, *, selected_pane_id, active_pane_id):
        ...
        n_swaps = sum(1 for c in self.calls if c[0] == "swap")
        if self.fail_all_swaps or (self.fail_first_swap and n_swaps == 1):
...
Run: `python -m unittest tests.test_cockpit_three_layer -v 2>&1 | tail -5`
...
Run: `python -m unittest tests.test_cockpit_three_layer tests.test_stage11_operator_cockpit -v 2>&1 | tail -5`
Expected: PASS（全綠）
...
Run: `python -m unittest tests.test_cockpit_three_layer.EnterSingleAuthorityTests -v`
...
Run: `python -m unittest tests.test_cockpit_three_layer -v 2>&1 | tail -5`
Expected: PASS；另跑 `python -m unittest tests.test_stage11_operator_cockpit -v 2>&1 | tail -5` 確認 Enter/swap 既有測試仍綠（若有直接呼叫 `app.on_key` 的舊測試，改為呼叫 `app.action_swap_selected()`）
...
Run: `python -m unittest tests.test_cockpit_three_layer.DoubleClickTests -v 2>&1 | tail -5`
...
class EventChainAndInvariantTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sole_listview_invariant_across_states(self):
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            def assert_sole():
                views = list(app.query(TextualListView))
...
Run: `python -m unittest tests.test_cockpit_three_layer.EventChainAndInvariantTests -v 2>&1 | tail -5`
...
Run: `python -m unittest tests.test_cockpit_three_layer.RestoreBeforeSwapTests -v 2>&1 | tail -5`
...
Run: `python -m unittest tests.test_cockpit_three_layer.JobsToggleTests -v 2>&1 | tail -5`
Expected: FAIL（`j` 未綁定）
...
同步更新既有 help 相關測試的預期文字（`grep -n "Enter swaps" tests/` 修正斷言）。

- [ ] **Step 2: 全測試套件**

Run: `python -m unittest discover -s tests -v 2>&1 | tail -8`
...
```bash
cd /home/paul_chen/prj_pri/paulshaclaw && PYTHONPATH=. python -m paulshaclaw.cockpit --cockpit-pane "$TMUX_PANE"
```
