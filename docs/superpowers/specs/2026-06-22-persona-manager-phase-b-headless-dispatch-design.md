...
**目標**
- 新增 `AgentLauncher` pluggable seam，提供 **copilot / claude / codex** 三個 headless 真實作（皆帶 remote 旗標、JSONL 輸出、cwd=worktree、autonomous 免確認）。
...
| remote | `--remote` | `--remote-control` | `--remote <ADDR>` / `remote-control` daemon |
