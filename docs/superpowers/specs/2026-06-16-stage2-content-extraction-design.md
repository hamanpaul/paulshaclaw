...
2. **#2 主修法＝atomizer 消毒斜線**：在 atomizer 把含 `/` 的 project 映射成 path-safe 元件（rich `project` 仍留 metadata）→ 對任意專案 robust、不再 skip。**輔以**把 serialwrap/OCP-0602/arcadyan 等補進 `~/.agents/config/projects.yaml` 取得乾淨 slug bucket。
...
| projects.yaml 補登 | `~/.agents/config/projects.yaml` | 補 serialwrap/OCP-0602/arcadyan 等活躍專案的 slug/roots/remotes | 設定 |
