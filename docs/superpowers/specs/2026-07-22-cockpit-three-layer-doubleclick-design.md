...
> 日期：2026-07-22 ｜ 狀態：修訂 v5（回應 codex round 4，待 round 5 重審）｜ 分支：`feature/cockpit-three-layer`
...
- 雙擊偵測（app 層 `on_list_view_selected`，即純滑鼠訊號）：
  ...
  - **手勢連續性範圍＝work-list 本地事件流**（明定）：非 work-list 的 `Selected` 不中斷手勢。此定義與「全域點擊流」定義在現有 UI 下**可證等價**——cockpit 全 UI 唯一的 ListView 即 work-list（`grep -n "ListView" cockpit/manager_panel.py cockpit/help.py` 零命中，2026-07-22 驗證；modal 場景另有 `_background_actions_blocked` 攔截＋開啟時清空 `_last_click` 雙重保險），不存在能產生非 work-list `Selected` 的畫面元件。**設計註記**：未來若新增第二個 ListView，此語意（不中斷 vs 中斷）需重新裁決。
