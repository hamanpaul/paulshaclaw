...
> 日期：2026-07-22 ｜ 狀態：修訂 v6（回應 codex round 5，待 round 6 重審）｜ 分支：`feature/cockpit-three-layer`
...
- 雙擊偵測（app 層 `on_list_view_selected`，即純滑鼠訊號）：
  ...
  - **手勢連續性範圍＝work-list 本地事件流**（明定）：非 work-list 的 `Selected` 不中斷手勢。此定義與「全域點擊流」定義在現有 UI 下**可證等價**——cockpit 全套件唯一的 ListView 實例化即 work-list：`grep -rn "ListView(" paulshaclaw/cockpit/*.py` 僅命中 `app.py:287`（`yield ListView(id="work-list")`；另一命中 `app.py:60` 為 textual 缺席時的 stub 類別定義、非實例化），2026-07-22 實跑驗證，涵蓋 app.py 與全部 modal 檔；modal 場景另有 `_background_actions_blocked` 攔截＋開啟時清空 `_last_click` 雙重保險。不存在能產生非 work-list `Selected` 的畫面元件。**設計註記**：未來若新增第二個 ListView，此語意（不中斷 vs 中斷）需重新裁決。
