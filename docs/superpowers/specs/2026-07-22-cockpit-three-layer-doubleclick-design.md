...
> 日期：2026-07-22 ｜ 狀態：修訂 v4（回應 codex round 3，待 round 4 重審）｜ 分支：`feature/cockpit-three-layer`
...
- **`Selected` producer 窮舉**：`_list_view.py` 全檔僅兩處 post `Selected`——`:273`（`action_select_cursor`，即 Enter binding 的 action）與 `:303`（滑鼠點擊鏈）。repo 內無任何程式化 `select_cursor` / `Selected` 呼叫點：`grep -rnE "select_cursor|list_view_selected|ListView\.Selected" paulshaclaw/ tests/ --include="*.py"` 零命中（exit 1；`-E` extended regex 交替、`.` 已跳脫，2026-07-22 實跑驗證）。⇒ 覆寫鍵盤 producer 後（§3.2），本 app 中 `Selected` 唯一來源＝滑鼠點擊。
...
- 雙擊偵測（app 層 `on_list_view_selected`，即純滑鼠訊號）：
  - **ownership guard 與手勢中斷語意**：`event.list_view.id != "work-list"` → 完全忽略、不碰任何狀態（別的 ListView 與 WORK 手勢無關）。`event.list_view` 是 work-list 但 `getattr(event.item, "pane_id", None)` 為 None，或點的是 ACTIVE 列 → **清空 `_last_click`**（視為手勢中斷：兩次配對點擊必須「連續」，中間插入無效點擊即重置）。pane_id 由 ListItem 建構時附掛（`WorkItem.pane_id`）。
...
- **脫離追蹤的殘留**來源與上限：(a) restore 失敗（終點 B）每次至多遺下一對——觸發條件罕見（快照後 pane 才死、tmux server 異常），且已 notify；(b) process 結束時未歸位的至多一對。跨 lifetime 累積上限 = restore 失敗次數 + 重啟次數。
...
| 5 | Candidate section includes all non-cockpit non-active panes | 保留 |
...
- **雙擊（單測，注入 clock）**：命中（<0.4s 同 pane_id）；逾時不觸發；跨列不觸發且重記首擊；成功後清空（三擊＝新首擊）；modal 開啟時被攔；**ownership guard**（`list_view.id` 非 work-list → 忽略且不碰狀態；work-list item 無 pane_id → 清空 `_last_click`）；**手勢中斷**（click C → click ACTIVE → click C 皆 <0.4s → 不觸發 swap，第三擊為新首擊）；**mismatch guard**（`state.selected_pane` 與 item 不符 → 降級新首擊）。
...
5. **點擊排隊壓縮（接受、列管）**：事件迴圈阻塞無硬上限，極端阻塞下同 pane 兩次慢擊可能被誤判為雙擊。影響：一次非預期但可逆的 swap（對象即使用者連點的 pane，下一次 activate 自動歸位）。不設 hard timeout／事件時間戳（§3.2 影響分析）。
