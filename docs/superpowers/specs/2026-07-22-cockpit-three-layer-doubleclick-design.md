...
> 日期：2026-07-22 ｜ 狀態：修訂 v3（回應 codex round 2，待 round 3 重審）｜ 分支：`feature/cockpit-three-layer`
...
- **`Selected` producer 窮舉**：`_list_view.py` 全檔僅兩處 post `Selected`——`:273`（`action_select_cursor`，即 Enter binding 的 action）與 `:303`（滑鼠點擊鏈）。repo 內無任何程式化 `select_cursor` / `Selected` 呼叫點（`grep -rn "select_cursor|list_view_selected|ListView.Selected" paulshaclaw/ tests/` 為空，2026-07-22 驗證）。⇒ 覆寫鍵盤 producer 後（§3.2），本 app 中 `Selected` 唯一來源＝滑鼠點擊。
...
- 雙擊偵測（app 層 `on_list_view_selected`，即純滑鼠訊號）：
  - **ownership guard**：`event.list_view.id != "work-list"` 或 `getattr(event.item, "pane_id", None)` 為 None → 直接忽略。pane_id 由 ListItem 建構時附掛（`WorkItem.pane_id`）。
...
- 點擊事件排隊壓縮：計時取樣在 handler 執行時而非事件發生時。本 app 全部同步 handler 的阻塞等級：swap（實測 ~2-5ms）、`list-panes` 單次 fork（~10ms 級）、`/proc` 讀（sub-ms）、`read_status` 檔案讀（ms 級）——皆 <50ms，遠小於 0.4s 閾值；加上 mismatch guard 兜底，接受不另處理。
...
- **雙擊（單測，注入 clock）**：命中（<0.4s 同 pane_id）；逾時不觸發；跨列不觸發且重記首擊；ACTIVE 列不記；成功後清空（三擊＝新首擊）；modal 開啟時被攔；**ownership guard**（`list_view.id` 非 work-list、item 無 pane_id → 忽略）；**mismatch guard**（`state.selected_pane` 與 item 不符 → 降級新首擊）。
...
- **restore（fake actions 錄呼叫序）**：happy path（restore→main 兩次 swap、記錄輪替）；被擠者死→跳 restore 直接 main；現任死→同；**C == displaced →恰好一次 swap（restore），無 `A↔A` 呼叫、記錄 None**（終點 A）；**restore 拋錯→恰好一次 swap 嘗試、無主 swap、記錄 None、notify**（終點 B），且下一次 activate 為乾淨 plain swap；main 拋錯→記錄 None+notify（終點 D）；連續 activate 三個 pane 的記錄輪替。
