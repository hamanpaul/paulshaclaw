...
> 日期：2026-07-22 ｜ 狀態：修訂 v7（回應 codex round 6，待 round 7 重審）｜ 分支：`feature/cockpit-three-layer`
...
- **唯一 ListView 不變量（run_test）**：`app.query(ListView)` 恰含一個實例且其 `id == "work-list"`（subclass 亦被 `query(ListView)` 匹配，與 WorkListView 相容）——把 §3.2 手勢範圍等價性的前提變成受測不變量；未來任何人新增第二個 ListView 即紅燈，強制執行 §3.2 的重裁決註記。
