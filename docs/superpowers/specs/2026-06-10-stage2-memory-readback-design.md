...
**Tree = path-style slug.** The slug doubles as the knowledge-layer path, so `knowledge/hamanpaul/paulshaclaw/`, `knowledge/work_prj/repo-a/`, `knowledge/work_prj/` (parent) form a tree naturally. Each node may have its own MOC; the brief uses the resolved node's MOC + Recent (parent-context folding is a follow-up).
