...
- 在 `obs-auto-moc` 內定義 file-watcher safety-net 介面契約；本 repo archive 只交付 hook/importer 端與冪等規則，watcher 實作另在 `obs-auto-moc` follow-up PR 落地。
...
> 本次 archive 的完成定義以 repo-local importer / hooks / installer / classifier / integration check 為主；watcher、歷史 corpus 大量回放、live hello-world smoke、24h burn-in 會在 follow-up 驗收。
...
- `install.sh` 在 `~/.agents/memory/hooks/.venv` 建一個 venv，並以**絕對路徑**執行 `.venv/bin/pip install -e <repo>` 把 `paulshaclaw.memory.importer` 套件安裝進去。`<repo>` 路徑由 `install.sh` 接收 `--repo-root <path>` 參數，預設為 `install.sh` 自身所在 repo（`git rev-parse --show-toplevel`）或已記住的 repo root。
- 升級流程：使用者 `git pull` 後跑 `~/.agents/memory/hooks/install.sh --upgrade`，內部執行 `.venv/bin/pip install -e <repo>`（與初次安裝相同指令，路徑由 install.sh 記住，不依賴使用者當前 cwd）。
...
### Safety net watcher 介面契約（在 obs-auto-moc 內 follow-up）

- 路徑：`obs_auto_moc/watchers/agents_inbox_watcher.py`（follow-up 新檔），與 systemd unit template `obs-auto-moc/systemd/agents-inbox-watcher.service` 一同在外部 repo 提交。
- 行為：
...
- 此 watcher 不屬於本 repo archive 的完成範圍；本 paulshaclaw spec 只規範介面與檔案路徑，實作 PR 在 obs-auto-moc 倉處理。
...
| sub-slice | Done 條件 |
|---|---|
| **A0 Hook smoke** | 本 repo 以 fixture-backed `test_hooks.py` 驗證 queue payload 與 installer；真實 CLI hello-world smoke 另作手動 follow-up |
...
| **A4 projects.yaml + watcher** | 本 repo 交付 `projects.yaml` 模板與 watcher 介面契約；watcher daemon 驗收另在 `obs-auto-moc` follow-up 執行 |
| **A5 E2E live** | 本 repo 僅保留 hook/importer 的 fixture-backed回歸；真實 CLI session + watcher 並行驗收另作手動 follow-up |
...
| `~/.agents/memory/runtime/queue/_failed/<key>.json` | 保留給後續 failure-routing slice 的預留位置 | future follow-up | 人工除錯 |
...
- `paulshaclaw/memory/tests/test_hooks.py`：驗證 hook queue write、Copilot history supplementation、install/uninstall 行為、editable install、config safety。
...
- [x] repo-local hook/install/uninstall/importer/classifier/idempotency/project-resolver 測試通過。
- [x] `paulshaclaw/memory/lint/frontmatter_lint.py` 與 importer output 的 repo-local fixture 回歸通過。
- [x] `paulshaclaw/memory/tests/stage2_integration_check.sh` 已擴充 importer dry-run on fixtures 並可通過。
- [x] OpenSpec delta 與 main spec 已同步到 repo-local importer MVP 範圍。
- [ ] 三家 CLI 的 live hello-world smoke（真實 hook host）待手動 follow-up。
- [ ] `obs-auto-moc` watcher、historical corpus backfill、classifier 大樣本 spot-check、24h burn-in 皆待外部/人工 follow-up。
