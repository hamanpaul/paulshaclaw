...
LLM 呼叫路徑（已驗證）：atomizer `AgentExecClient` → `scripts/claude-gemma4`（headless claude CLI，prompt 走 stdin）→ 本機 proxy `:18080` → gemma4 `gemma4-26b-a4b-nvfp4` @ 192.0.2.10。backend 常駐不下線；本機 `:8001` 與此路徑無關。
...
3. **gemma4 可用性靠既有基礎建設**。backend @ 192.0.2.10 常駐；claude-gem 可由 paulshiabro `/agent %x start` 在閒置 pane 拉起、`/agent` 查線上狀態；Phase 2 生命週期跟著 `scripts/start.sh`。fail-closed 已內建：gemma4 / proxy 斷線時該 session 留在 split、下輪 dream 重試，不掉資料、不阻塞。
...
flowchart LR
    subgraph Phase2a[Phase 2a forward-only canary]
      ...
      G --> PX[proxy :18080] --> GM[gemma4 @ 192.0.2.10]
