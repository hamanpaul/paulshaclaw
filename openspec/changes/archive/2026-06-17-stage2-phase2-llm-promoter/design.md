...
LLM 路徑：`AgentExecClient`（stdin 餵 prompt）→ `scripts/claude-gemma4`（headless claude）→ proxy `:18080` → gemma4 `gemma4-26b-a4b-nvfp4` @ 192.0.2.10（常駐）。fail-closed 與 LLM 輸出 cache 凍結已內建。
