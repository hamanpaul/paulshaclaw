...
- Add config-driven Copilot account support so runtime accounts are not hardcoded; sample accounts can map `hamanpaul -> haman` and `org-a -> arc`, but production config may define zero, one, or many accounts.
