...
- [x] 2.14 skipped 模式（無 `--footer` 且 `no-tty`／`plan-only`）的 `footer_selection` 省略 `enabled` 欄位：operator 裁決，因 skipped 不讀 `paulshaclaw.yaml`，偏離 spec R5 字面為已接受的有界殘餘。
- [x] 2.15 footer failure reason repair：對齊 code-review-403 與 operator adjudication，讓 `_emit_footer_failure()` 在未帶 `--footer` 的單一 JSON failure report 仍回報 `skipped.reason`（`no-tty` / `plan-only`），同步移除 `prepare_footer_selection()` 已未使用的 `verify` 參數並補 focused regression；archive／PR／關 issue 仍由 Manager / operator 後續執行。
...
- [x] 2.18 reviewer follow-up：補齊 `agy` `accounts[]` 以及多帳號 `collect_agy` 對齊（含 disabled account 過濾），修正 `footer_select.py` 匯入例外型別、`collect_all` 用 `config.agy.enabled` guard、以及 `agy` 的 `local_observed+percent_used` 著色改用 `classify_usage`（保留 `~` 前綴）；並同步修正文檔 R8 的 changelog 檔名。
