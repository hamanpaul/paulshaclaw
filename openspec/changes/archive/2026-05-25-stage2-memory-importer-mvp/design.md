...
- Define the `obs-auto-moc` watcher interface contract so a follow-up external implementation can recover missed hooks without expanding this repo-local archive scope.
...
To eliminate PYTHONPATH and `python3` ambiguity, `install.sh` creates `~/.agents/memory/hooks/.venv` and runs `.venv/bin/pip install -e <repo>`. All hook config templates invoke `~/.agents/memory/hooks/.venv/bin/python` directly. If venv is missing, hook scripts fall back to "write queue payload only" + WARN log; a future watcher integration can catch up.
...
### Failure routing reserved for follow-up

`~/.agents/memory/runtime/queue/_failed/` remains part of the canonical tree, but this repo-local archive does not yet relocate failures into that subtree. The path is reserved for a later failure-routing slice.
...
| 5 | Land the external `obs-auto-moc` watcher follow-up and verify it catches an artificially-missed session. |
| 6 | Run live 24h burn-in with hook + watcher both enabled as manual follow-up validation. |

Rollback: remove hook config files (or run `uninstall.sh`). Inbox content is preserved; downstream sub-specs can continue from existing data. If/when the external watcher lands, stop that systemd unit separately.
