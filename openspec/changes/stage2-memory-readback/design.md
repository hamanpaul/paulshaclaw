...
- **D3 — Tree = path-style slug.** The project slug doubles as the knowledge-layer path, so multi-repo workspaces nest naturally (`knowledge/arc_prj/repo-a/`). Alternative (separate "group" metadata) adds a parallel structure for no near-term gain.
