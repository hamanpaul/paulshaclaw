...
The service reads a global config that declares one or more workspace roots. Defaults are `~/prj_work` and `~/prj_pri`. The config schema is additive and forwards-compatible so that other stages may later contribute their own sections to the same file.
