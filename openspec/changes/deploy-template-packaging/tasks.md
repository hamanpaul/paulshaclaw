...
- [x] pre-archive review repair：preflight 使用獨立 test-runtime resolver（無 `.venv`／`PSC_PYTHON` 時可採用具備 pytest 的系統 Python），先檢查 build 並在 pip 失敗時給可操作訊息；禁止 `apply_install_plan` 靜默忽略 `python_exe`；補齊缺模板下 status／uninstall --apply 與 system-Python preflight 回歸測試；縮小 archive checker 開檔例外範圍；移除 CI 重複安裝 build
