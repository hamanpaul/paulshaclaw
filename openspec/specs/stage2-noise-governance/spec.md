...
- **WHEN** slice body 去除標題行與空白後無實質內容（如 `# Session <uuid>`），或含 `(無內容)` / `尚未收到您的具體需求` / `目前尚未收到` 等 placeholder 字串
