...
Template rename 規則 MUST 支援 `__INSTANCE__` 取代為實例名，且 MUST 移除 `.tmpl` 後綴，讓目標檔名可直接落到部署目錄。

#### Scenario: template 資產涵蓋 core/state/secret

- **WHEN** 操作者執行 `python3 -m unittest tests.test_stage7_deploy_three_plane.TemplateMappingTests -v`
- **THEN** 測試 MUST 驗證 template 清單至少含四個檔案、plane 集合為 `core/state/secret`，且每個 template 實體檔案存在
