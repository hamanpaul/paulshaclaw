...
Stage 8 SHALL load Copilot accounts from configuration. Runtime behavior MUST support zero, one, or many accounts and MUST NOT hardcode `hamanpaul`, `paulc-arc`, `haman`, or `arc`. Each account MAY define `id`, `label`, `kind`, `monthly_allowance`, and provider-specific billing scope such as `org` or enterprise identifiers.
